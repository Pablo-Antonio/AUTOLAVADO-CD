...
function agregarCarrito(idServicio) {
  $.ajax({
    method: "GET",
    url: "/apiServicios/show/" + idServicio,
  })
    .done(function (data) {
      //console.log(data);
  ...
  //var data = $("#tableSerVender tbody").find("tr").eq(0).text();
  //console.log(data);
}
...
function validarServicio(datos, bnt) {
  ...
  if (!ban) {
    ...
    /*tableVenCaj.row
      .add([datos.idServicio, datos.nombre, 1, datos.precio, bnt])
      .draw(false);*/
  }
}
